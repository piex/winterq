...
static JSValue js_event_constructor(JSContext *ctx, JSValueConst new_target, int argc, JSValueConst *argv) {
  ...
  // 创建JS对象
  obj = JS_NewObjectClass(ctx, js_event_class_id);
...
void js_init_event(JSContext *ctx) {
  ...
  // , event_target_class_id, custom_event_class_id
  ...
  event_class = JS_NewCFunction2(ctx, js_event_constructor, "Event", 1, JS_CFUNC_constructor, 0);
